...
the Born stability check. Each run is cached, keyed by a hash of its inputs.
The cache is invalidated whenever the MEAM file changes, so re-running a
composition after a refit recomputes rather than returning a stale number.
Figure~\ref{fig:meam_app} shows the interface on a pure-aluminium run.
