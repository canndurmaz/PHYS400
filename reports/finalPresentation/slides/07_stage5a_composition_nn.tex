% =====================================================================
% Slide 11: Stage 5a -- composition NN surrogate
% =====================================================================
\begin{frame}{Stage 5a: Composition $\to (E, \nu)$ Neural Surrogate}
\fwtag{TensorFlow / Keras}
\begin{columns}[T]
\column{0.42\textwidth}
\textbf{Architecture.}
$12 \!\to\! 32 \!\to\! 20 \!\to\! 2$ fully-connected MLP\\
with ReLU activation; Huber loss on $\hat C_{11}, \hat C_{12}$;\\
deep ensemble of five seeds~\citep{lakshminarayanan2017}
for epistemic uncertainty.

\medskip
\begin{block}{Result}
\small
Validation Set (30\,\% of the dataset): $R^{2}_{E} = 0.77$,\ $R^{2}_{\nu} = 0.52$.\\[0.3em]
\textbf{Al\,7075 Example:} $\hat E = 75.5$\,GPa,\ $\hat\nu = 0.334$\\
\scriptsize \quad(published: $71{-}72$\,GPa,\ $\nu \approx 0.33$ --- within $\sim\!5\,\%$).
\end{block}

\smallskip

\column{0.56\textwidth}
\centering
\includegraphics[width=0.99\linewidth]{figures/ml_app.png}\\[-0.2em]
\scriptsize Interactive composition predictor.
\end{columns}
\end{frame}
