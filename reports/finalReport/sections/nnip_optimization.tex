...
$0.29$ to $0.105$ over roughly $2300$ training steps
...
\citep{kingma2015adam} descending on the normalized parameter vector. The search
...
LAMMPS and comparing against the MD targets. The outcome is mixed
...
$E$ and $19.6\,\%$ on $\nu$ (medians $36\,\%$ and $11\,\%$); 8 of the 25 clear
the $20\,\%$ tolerance on $E$ and 17 on $\nu$, with the same 8 clearing both. The stable, in-tolerance cases cluster in the
...
the external validation of Section~\ref{sec:validation} quantifies. The 35
rejected alloys show most clearly where the method needs more work, and they
set the agenda in Section~\ref{sec:future}.
Figure~\ref{fig:nnip_timings} breaks down the wall-clock cost of one full
cycle: the LAMMPS sampling dominates while the surrogate training and inverse
optimization are comparatively cheap, which is the whole reason for fitting a
surrogate rather than optimizing against LAMMPS directly.
...
\begin{figure}[htbp]
	...
	\begin{minipage}[t]{0.48\textwidth}
		...
		\caption{Surrogate Huber loss over $\sim$2300 training steps, falling
		from $0.29$ to $0.105$.}
		\label{fig:nnip_loss}
	\end{minipage}
	\hfill
...
\begin{figure}[htbp]
	...
	optimization are comparatively cheap.}
